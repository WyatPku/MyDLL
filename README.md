...
Botzone上传演示（无需手动改代码）：http://v.youku.com/v_show/id_XMTgzNTMyOTUzNg==.html
...
# 重大更新
近有了重大更新：代码直接复制上Botzone，而无需修改！甚至你不需要JSON库就可以本地调试＋上传无误！
实现原理就是预编译语句。这样本地调试可以完全取代网上调试了，甚至在Debug和测试性能方面比网上强不少，
因为有着对过程中标准输出（cout、printf等）的支持、批量测试的支持（比如每次测试100组，哪组有问题还可以复现过程）
、调节速度的支持！
...
11月52日更新：不需要手动改代码，本地调试以后直接上传Botzone，方便快捷。具体方法见演示视频。
